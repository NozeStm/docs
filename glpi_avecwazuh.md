...
Un outil ITSM (**Information Technology Service Management**) est une solution logicielle qui aide les entreprises à gérer l'ensemble des services informatiques[cite: 8]. Cet outil permet de planifier, concevoir, déployer, exploiter et optimiser les services informatiques[cite: 9].

### 1.2 Rôle de l'ITSM en Cybersécurité

L'utilisation d'un outil ITSM en cybersécurité permet[cite: 12]:

  * **La réduction des délais de réponse**[cite: 13].
  * **L'amélioration de la coordination et communication**[cite: 14].
  * **D'apporter une traçabilité** pour les audits et les analyses post-incident[cite: 15, 25].
  * **La standardisation des processus** via des modèles prédéfinis[cite: 26].

### 1.3 Présentation de GLPI

**GLPI** (**Gestionnaire Libre de Parc Informatique**) est un logiciel open-source de gestion des services informatiques (ITSM) et des actifs[cite: 32]. Son module de gestion des tickets est au cœur de son fonctionnement [cite: 33], permettant d'enregistrer, prioriser, assigner et suivre les incidents[cite: 34].
...
Le serveur **SRV-GLPI** a l'adresse IP `172.16.0.8` [cite: 89] et le serveur **SRV-WAZUH** a l'adresse IP `172.16.0.7` [cite: 75, 79] dans le réseau LAN SODECAF (`172.16.0.0/24`)[cite: 73]. L'accès à GLPI se fait via `http://172.16.0.8/`[cite: 89].

  * **Identifiant Administrateur GLPI :** `glpi` [cite: 91]
  * **Mot de passe GLPI :** `glpi` [cite: 92]

-----

## 3\. Configuration de l'API GLPI

Wazuh utilise l'API de GLPI [cite: 94] pour créer des tickets, nécessitant un jeton d'application (`app_token`) et un jeton utilisateur (`user_token`) pour l'authentification[cite: 96].
...
Le script Bash est utilisé pour automatiser la génération des tickets incidents depuis Wazuh[cite: 213].

**Extraits du script avec les jetons mis à jour[cite: 221]:**
...
Les étapes d'installation et de configuration des droits du script (propriétaire `root:wazuh`, droits `750`) et du fichier de log (`/var/log/glpi_ticket.log`) doivent être respectées[cite: 270, 271, 273].

### 5.2 Configuration de la Réponse Active Wazuh

Le fichier de configuration de Wazuh doit être modifié pour déclencher le script `glpi_ticket.sh` lors d'une alerte SSH (rule.id=`5763`)[cite: 291].
...
Le service `wazuh-manager` doit ensuite être redémarré[cite: 292].

### 5.3 Test Réel de la Création de Ticket

Effectuer une attaque sur le service ssh du serveur DVWA avec Kali Linux [cite: 304] et vérifier la création du ticket incident dans GLPI[cite: 305].

-----

## 7\. Gestion des Tickets Incidents 🛠️

La dernière phase consiste à traiter les tickets qui ont été générés automatiquement par Wazuh[cite: 318].

### 7.1 Connexion et Rôles

  * Connectez-vous à l'interface GLPI avec le compte `SOC-N1`[cite: 319].
  * Si nécessaire, vérifiez et ajustez les droits de l'utilisateur `SOC-N1` en vous connectant avec le compte administrateur `glpi`[cite: 319].

### 7.2 Manipulation des Incidents

À partir du tableau de bord de GLPI ou de la liste des tickets, l'analyste N1 du SOC effectue les actions suivantes pour gérer l'incident (par exemple, un ticket **Alerte Wazuh - Tentative de brute force SSH**)[cite: 282]:

1.  **Réponse / Prise en Charge** :

      * L'analyste prend connaissance de l'alerte (date, règle ID, agent, IP Source, gravité)[cite: 284, 285, 287, 288, 289].
...
L'objectif est de vérifier le bon fonctionnement du processus ITSM complet, de la détection (Wazuh) à la résolution et la clôture (GLPI)[cite: 319].
